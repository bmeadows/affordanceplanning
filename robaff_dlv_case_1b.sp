% Affordance planner (version for testing claim 1b, plan generation with all forbidding affordances removed)

% Name:          robaff_dlv_case_1b.sp
% Author:        Ben Meadows
...
% java -jar sparc.jar robaff_dlv_case_1b.sp -solver dlv -A > plans_out.txt
...
% Cannot pick up a standard-fragility object with gripper_arm_actuation=pneumatic
...
% Cannot pick up a standard-fragility object with gripper_arm_actuation=pneumatic UNLESS gripper_articulation=flexible_conform
...
% Cannot pick up a fragile object with a pneumatic arm actuation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% forbidding_aff(id31, pickup(R, F, O)).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% affordance_fails(id31, pickup(R, F, O), I) :- -ob_fragility(O, fragile). % In this circumstance the forbidding affordance fails (doesn't apply)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% affordance_fails(id31, pickup(R, F, O), I) :- -gripper_arm_actuation(R, pneumatic). % In this circumstance the forbidding affordance fails (doesn't apply)

% Cannot pick up a heavy object with 'flexible_conform' gripper articulation
...
% Cannot pick up a heavy object with weak force
...
user_weight(risk, moderate).
user_weight(energy, moderate).
...
holds(loc(per0, rm1), 0).
holds(loc(per1, rm2), 0).
holds(d_state(door0, open), 0).
holds(d_state(door1, closed), 0).
holds(d_state(door2, locked), 0).
holds(d_state(door3, closed), 0).
holds(d_state(door4, closed), 0).

% Statics (objects)
ob_weight(text0, light).
ob_fragility(text0, fragile).
ob_weight(text1, light).
ob_fragility(text1, standard).
ob_weight(text2, middle).
...
d_weight(door0, light).
d_weight(door1, light).
d_weight(door2, light).
d_weight(door3, light).
d_weight(door4, light).
...
gripper_finger_number(rob0, 4). % 3 possibilities
gripper_arm_actuation(rob0, pneumatic). % 2 possibilities
gripper_articulation(rob0, flexible_conform). % 2 possibilities
% $
...
%goal(I) :- holds(loc(rob0, rm1), I).

goal(I) :- holds(loc(TB, rm1), I), #textbook(TB).
